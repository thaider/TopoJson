(function ($) {
			...
			function filter_geometry_bv( data ) {
				...
				for ( var i = 0; i < geometries.length; i++ ) {
					if ( typeof bedarfsverkehre[geometries[i].properties.name.toLowerCase()] !== 'undefined' ) {
						geometries_filtered[gfi] = geometries[i];
						bv = bedarfsverkehre[geometries[i].properties.name.toLowerCase()];
			...
			function show_tooltip(data, mouse_evt) {
				...
				if( tt.hasClass( 'clicked' ) === false ) {
					tt.find('.bv_tooltip_ort').html( data.name );
					...
					d3.select( 'g.map-at-municipalities #gid-' + data.gid ).classed( 'highlight', true );
				}
			}
...
//				bedarfsverkehre_ges = $.extend( {}, bedarfsverkehre, bedarfsverkehre_eing );
				...
				d3.json( wgServer + wgScriptPath + "/extensions/TopoJson/json/oesterreich.json", function(error, aut) {
					...
					d3.json( wgServer + wgScriptPath + "/extensions/TopoJson/json/gemeinden.json", function(error, aut_gem) {
